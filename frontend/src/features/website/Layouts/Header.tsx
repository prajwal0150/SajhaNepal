...
import { HeartHandshake, Menu, X, Globe, CircleUserRound, PenLine, TriangleAlert, Radio } from 'lucide-react';
import { Link, NavLink } from 'react-router-dom';
import { cn } from '@shared/utils/format';

const NAV = [
  { to: '/', label: 'Home', end: true },
  { to: '/map', label: 'Live Relief Map' },
      ...
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-2 px-3 sm:px-4">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <button className="inline-flex items-center gap-1 rounded-md px-2 py-1.5 text-[13px] font-medium text-ink/70 hover:bg-ink/5">
            <Globe size={14} aria-hidden /> EN <span className="text-muted">|</span>{' '}
            <span className="text-primary">नेपाली</span>
          </button>
          <Link
            to="/login"
          ...
          >
            <CircleUserRound size={15} aria-hidden /> Login / Register
          </Link>
          <Link
            to="/report"
...
export const Header = WebsiteHeader;
