...
import { HeartHandshake, Menu, X, Globe, CircleUserRound, TriangleAlert } from 'lucide-react';
      ...
      <div className="bg-critical text-white">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-x-3 gap-y-1 px-3 py-1.5 text-[11px] leading-tight sm:text-xs">
          <span className="inline-flex items-center gap-1.5 font-semibold">
            ...
            <span className="rounded bg-white/20 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide">Priority flash bulletin</span>
          </span>
          <p className="min-w-0 flex-1 truncate sm:whitespace-normal">
            Heavy rainfall has triggered landslides in Sindhupalchok and Rasuwa districts. Stay safe and follow official updates.
          </p>
          <span className="ml-auto hidden items-center gap-3 text-white/90 md:flex">
            <span className="inline-flex items-center gap-1"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-white" /> Last updated 30 mins ago</span>
            <span className="inline-flex items-center gap-1">⚠ JNDMPHA Sync Active</span>
          </span>
        </div>
      </div>
        ...
        {open && (
          <nav className="animate-fade-up border-t border-ink/10 bg-white px-3 py-2 lg:hidden" aria-label="Mobile">
            ...
            <div className="mt-2 flex gap-2 border-t border-ink/10 pt-2">
              <Link to="/login" className="flex-1 rounded-lg border border-primary/30 px-3 py-2 text-center text-sm font-semibold text-primary" onClick={() => setOpen(false)}>
                Login
              </Link>
              <Link to="/report" className="flex-[2] rounded-lg bg-primary px-3 py-2 text-center text-sm font-bold text-white" onClick={() => setOpen(false)}>
                REPORT A NEED
              </Link>
            </div>
          </nav>
