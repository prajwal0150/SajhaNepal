import type { ServicesStats } from '../types/servicesTypes';
import { Building, HandHelping, HeartPulse, MapPinned } from 'lucide-react';

export function ServicesStats({ stats }: { stats: ServicesStats }) {
  const cards = [
    { icon: MapPinned, value: `${stats.provinces}`, label: 'Provinces covered', tint: 'bg-primary/10 text-primary' },
    ...
    { icon: HeartPulse, value: format(stats.needsVerified), label: 'Needs verified', tint: 'bg-critical/10 text-critical' },
  ] as const;

  return (
    <section className="mx-auto max-w-7xl px-3 sm:px-4" aria-label="Services by the numbers">
